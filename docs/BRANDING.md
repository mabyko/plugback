...
* **메뉴바 글리프**: 맥북 외곽선(화면 + 하판 쐐기 + 상단 노치) 안에 창 하나. 단색 템플릿 이미지.
  움직이지 않는다. (복원 순간 1회 펄스는 구현되면 그때 이 줄에 적는다.)
* **앱 아이콘**: 같은 맥북 도상에 창 3장이 겹쳐 놓인 형태. 앞 1장만 앰버, 뒤 2장은 흰색 저투명도라
  겹침이 그대로 비친다 — 색 하나로 "이 창 하나만 움직인다"를 말한다.
  노치는 맥북임을 밝히는 유일한 디테일이다. (원본 assets/appicon.svg)
...
* 맥북 도상(노치 + 쐐기 하판)은 아이콘·글리프·[hero.svg](../assets/hero.svg) 세 자산이 같은 비율을 쓴다 —
  노치 너비는 화면 너비의 19%, 화면과 하판 사이 간격은 하판 너비의 2%.
