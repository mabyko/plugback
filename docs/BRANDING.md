...
* 자산의 악센트는 전기 앰버 `#FFB000` 한 색이다. 카드 UI는 Graphite(바이올렛), Porcelain(블루), Coral(코랄), Sage(세이지) 중 선택한 팔레트를 쓴다. 각 팔레트는 라이트·다크를 제공하며, 레이아웃과 독립적이다. 글꼴은 macOS 시스템 글꼴을 쓴다. 카드의 Button·Toggle은 시안의 모서리·치수·팔레트를 명시하고, Toggle의 접근성 표현은 표준 컨트롤로 제공한다. 설정 창은 네이티브 Form을 유지한다.
