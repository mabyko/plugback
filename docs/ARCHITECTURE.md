...
- **인터페이스**: `windows(of: 대상 앱들) -> [창 정보]`, `move(창, to: 좌표) -> 실제 좌표`, `unminimize(창) -> 성공 여부`, `isRunning(앱)`, `openWindow(앱) async -> 창 존재 여부`. 실행 여부·새 창 열기는 건너뜀 사유의 구분과 예외 옵션에 필요하며, 창 열거와 같은 앱 집합을 봐야 하므로 창 세계의 일부로 둔다. 화면 열거는 여기 속하지 않는다 — ScreenProvider의 일이다
