...
- **숨기는 것**: **신호원이 둘이라는 사실 전부.** 창 이동(직접 옮긴 것)과 앱 전환(옵저버가 못 받는 앱·나중에 켠 앱)이 서로를 메우지만, 컨트롤러가 아는 것은 「수집할 때가 됐다」 하나다. 두 압축 정책(전환 스로틀 / 이동 디바운스)과 등록 대상 갱신이 여기 산다.
- 어댑터가 실제로 둘이라 이 seam은 지어낸 것이 아니다. 세 번째 신호원이 생겨도 컨트롤러 쪽은 바뀌지 않는다.
- 주기 타이머가 아니다 — 사용자가 앱을 만질 때만 발화하므로 자리를 비우면 조용하다(F-07).
- 실험실이 꺼져 있으면 컨트롤러가 아예 만들지 않는다. 꺼진 기능이 알림을 받고 있으면 "꺼짐"이 아니다.
- **ActivityWatcher**(앱 전환 압축)는 이 모듈의 구현 세부다 — 인터페이스에 나오지 않는다.

### ProfileSlots

- **인터페이스**: `source(for:)` / `resolved(for:)` / `targets(for:)` / `all` / `capture` / `collect` / `confirm` / `edit` / `remove` + 실험실 토글.
- **숨기는 것**: 화면 하나가 슬롯 둘을 갖는다는 사실 전부 — 키 규약, 복원 소스 판정(더 최근 것·동점은 수동), 씨앗 복사, 수집 후보의 수명, 확정, 병합, 실험실 on/off가 후보 판정에 미치는 영향, 읽지 못한 파일에 쓰지 않는 금지(F-04.2).
