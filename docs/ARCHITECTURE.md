...
- **격리 계약**: 심의 모든 메서드는 async이고, **구현이 자기 실행 흐름을 소유한다**. 실물 어댑터는 actor — AX 왕복(호출당 250ms 한도)이 메인 액터를 막지 않는다 (F-02.4). NSWorkspace 읽기만 메인으로 홉한다. StrictConcurrency가 이 계약을 컴파일 타임에 지킨다.
