...
- 네이티브 밀도: A–D의 헤더는 화면 이름·연결 상태와 프로필 요약 두 줄을 공유한다. B의 큰 아이콘·슬로건·별도 숫자 영역은 생략한다. 행 기본 높이는 30–34pt, 동작 버튼은 최소 32pt이며 글자 크기에 따라 늘어난다. `CardScrollView` 안에 스크롤바용 오른쪽 여백을 두어 목록과 결과 상세의 내용 겹침을 막는다. 제외 앱의 화면별 펼침 상태는 카드의 로컬 상태이며 프로필에 저장하지 않는다. 화면이 하나면 복원 소스는 동작 영역에 고정하고, 여러 화면이면 각 섹션의 앱 목록 앞에 둔다.
...
**M1–M5 구현 완료 (2026-08).** 남은 것: §2 MenuBarUI의 실기기 검증 항목들, 아래 화면 식별자 스파이크. (2026-09에는 B를 기본값으로 두고 A–D 레이아웃과 독립 색상 선택을 제공한다.)
