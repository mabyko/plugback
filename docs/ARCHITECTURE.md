...
- **인터페이스**: 관찰 가능한 상태(현재 화면, 프로필 유무, 마지막 복원 결과, 복원 진행 중, 권한 상태, 복원 모드) + 명령(저장, 복원(async — 반환 시점 = 완료 시점), 대상 앱 토글·삭제, 모드 변경) + 카드 열림 통지(`cardOpened` — 상태 동기화와 일회성 저장 확인 만료). **명령은 화면 상태를 스스로 동기화한다** — 호출 순서 의식이 없다.
