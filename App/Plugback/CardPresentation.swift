...
enum CardPresentation {
    ...
    ///
    /// 행이 이미 셋을 말한다 — 흐린 색(안 중요함), 「추가」 버튼(할 수 있는 일),
    /// 위 체크 목록과의 대비. 머리말에 남은 일은 "왜 여기 따로 있나" 하나고, 답은 저장이 안 돼서다.
    ///
    /// 쓸 수 없는 말들: 「건너뜀」은 복원 대상인데 안 옮기는 것이고(CONTEXT 정의어) 이 앱은
    /// 애초에 대상이 아니다. 「그대로 둠」은 「제자리」와 겹친다. 「미등록」은 대상 앱의
    /// 금지어(등록 앱)에 스친다. 「신규」는 거짓이다 — 오래 쓴 앱도 저장 안 했으면 여기 온다.
    static let untrackedHeader = "저장되지 않은 앱"
