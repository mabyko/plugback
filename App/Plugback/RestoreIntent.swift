...
/// 잠금 화면에서는 실행하지 않는다 — 시스템이 실행 전 기기 잠금 해제를 요구한다 (D8). 별도 예약 기능은 두지 않는다.
struct RestoreLayoutIntent: AppIntent {
    static let title: LocalizedStringResource = "지금 레이아웃 복원"
    static let description = IntentDescription("현재 작업 환경의 마지막 저장본대로 대상 앱의 창을 복원합니다. 저장하지 않은 창은 건드리지 않습니다.")
